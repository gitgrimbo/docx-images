...
class EntryHandler {
  ...
  async handleImage(entry, readStream) {
    ...
    const allResults = await Promise.all(occurencesOfImageInBook.map(async (image, i) => {
    ...
    const cropped = allResults.filter((result) => result && result.outputPath);

    if (cropped.length > 0) {
      console.log(id, entry.fileName, `image was cropped to:`);
      cropped.forEach((crop, i) => {
        console.log(`  [${i}] ${crop.outputPath}, old-size: ${JSON.stringify(crop.old)}, new-size: ${JSON.stringify(crop.new)}`);
      });
    } else {
